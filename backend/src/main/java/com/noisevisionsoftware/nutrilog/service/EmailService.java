...
public class EmailService {
    ...
    public void sendVerificationEmail(NewsletterSubscriber subscriber) {
        try {
            ...
            String emailContent = templateEngine.process("email/verification-email-content", new Context(null, contentVars));

            // Przygotuj zmienne dla szablonu bazowego
            Map<String, Object> baseVars = new HashMap<>();
            baseVars.put("emailContent", emailContent); // Wstrzykujemy już wyrenderowaną treść
    ...
    public void sendWelcomeEmail(NewsletterSubscriber subscriber) {
        try {
            ...
            // Renderuj treść emaila
            String emailContent = templateEngine.process("email/welcome-email-content", new Context(null, contentVars));
    ...
    public void sendTemplatedEmail(String to, String subject, String templateName, Map<String, Object> variables) {
        try {
            ...
            String emailContent = templateEngine.process("email/" + templateName + "-content", new Context(null, variables));
